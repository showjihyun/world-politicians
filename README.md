...
- **Evidence is machine-filtered, not human-verified.** The LLM judges headlines, not article bodies, so marginal calls get through. Treat a linked article as supporting context, not as a citation someone checked. Recent coverage comes from Google News, older events from the GDELT archive (which reaches back to 2017) — the 48 remaining are mostly pre-2017 relationships that neither source can reach.
