 ...
 * 그래프 도메인 — 순수 함수만 둔다.
 *
 * 여기서 데이터셋(RELATIONSHIPS 등)을 import 하지 않는 것이 이 파일의 전부다.
 * 예전에는 buildGraph 가 모듈 전역 RELATIONSHIPS 를 직접 읽었고, 그래서
 * "관계 3개짜리 그래프" 같은 최소 입력으로 검증할 방법이 없었다. 인자로 받으면
 * 테스트가 자기 데이터를 넣을 수 있고, 실제 데이터셋 결합은 lib/graph.ts 가 맡는다.
 *
 * 규칙: 이 파일은 ../types 외의 런타임 import 를 갖지 않는다.
 */
...
export function buildGraph(politicians: Politician[], relationships: Relationship[]) {
