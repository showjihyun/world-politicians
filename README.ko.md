...
- **근거는 기계가 걸러낸 것이지 사람이 검증한 것이 아닙니다.** 두 인물이 함께 등장하는 기사를 모은 뒤, 제목에 둘 사이의 상호작용이 드러나지 않는 것을 LLM 이 제거했습니다(후보 802건 중 52% 제거). 결과는 확연히 나아졌지만("트럼프·바이든 시기 40조 부채" 기사가 더 이상 두 사람의 갈등 엣지에 붙어 있지 않습니다) 제목만으로 판단한 것이라 애매한 건이 남습니다. **266개 중 208개에 근거가 있고, 58개는 없으며 없다고 표시됩니다** — 대부분 최근 뉴스 검색으로는 닿지 않는 오래된 관계입니다.
