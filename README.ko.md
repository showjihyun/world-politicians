...
- **소스 구성이 쏠려 있고, 통신사는 사실상 없습니다.** 아카이브 전체는 18개 매체에서 왔지만 상위 5곳이 69%를 차지하고, AP 와 Reuters 를 합쳐도 4.9% 입니다 — 분류기가 무엇을 보게 되는지를 몇 개 정치 전문 매체가 정합니다. 인사이트 패널이 이 구성을 감추지 않고 그대로 보여주는 이유는, 이 선별이 LLM 이 잘하고 못하고보다 앞단에 있기 때문입니다.
